...
const DrinkArray = ({ filter, drinkList, adminKey, getDrinkList, setShowLoader, menuSettings, editMenu, showMenuDesc}) => {
        ...
        <>
            ...
            {filteredList.map((drink) => {
                if (!filter.tags || drink.tagCount === filter.tags.length) {
                    return <DrinkEntry drink={drink} getDrinkList={getDrinkList} adminKey={adminKey} filteredTags={filter.tags} setShowLoader={setShowLoader} menuSettings={menuSettings} editMenu={editMenu} showMenuDesc={showMenuDesc}/>
                }
            })}
